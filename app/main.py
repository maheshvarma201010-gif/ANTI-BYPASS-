...
def deep_url_inspect(raw_str: str) -> tuple[bool, str]:
    ...
    dangerous_patterns = [
        "data:text/html", "blob:", "about:blank",
        "javascript:", "document.write", "document.open",
        "top!==self", "eval(", "decodeuricomponent",
        "<script", "<style>", "<a id="
    ]
    for pattern in dangerous_patterns:
        if pattern in full_text:
            return True, f"Dangerous pattern '{pattern}' detected"
...
async def security_firewall_middleware(request: Request, call_next):
    ...
    # ========== BLOCK EMPTY REFERER ON SHORT LINKS ==========
    # This is the CRITICAL fix - blocks the bookmarklet redirect
    if re.match(r"^/[a-zA-Z0-9_-]{6,}$", path) and (not raw_referer or not raw_referer.strip()):
        is_bypass = True
        bypass_reason = "Blocked: Empty Referer on short link (Bookmarklet/Nicktrick bypass attempt)"
        logger.warning(f"🛡️ EMPTY REFERER BLOCKED: IP={client_ip}, Path={path}")

    # ========== BLOCK NICKTRICK IN QUERY ==========
    if not is_bypass:
    ...
    # ========== BLOCK EXTERNAL DOMAINS IN REFERER ==========
    if not is_bypass and raw_referer:
        blocked_domains = ["urllinkshort.in", "antibypass-31lh.onrender.com"]
        for domain in blocked_domains:
            if domain in raw_referer.lower():
                is_bypass = True
                bypass_reason = f"Blocked known bypass domain in referer: {domain}"
                break
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .premium-card {
            ...
            box-shadow: 0 40px 100px -30px rgba(0, 0, 0, 0.95), 0 0 60px -10px rgba(239, 68, 68, 0.2), inset 0 1px 1px rgba(255, 255, 255, 0.05);
        ...
        .shield-svg { width: 44px; height: 44px; fill: #ef4444; filter: drop-shadow(0 0 12px rgba(239, 68, 68, 0.6)); }
        .shimmer {
            font-size: 26px;
            font-weight: 800;
            margin: 0 0 16px 0;
        ...
        @keyframes pulse { 0%, 100% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.25); opacity: 0.4; } }
        .desc-text { color: #e4e4e7; font-size: 16px; line-height: 1.6; margin: 0 0 32px 0; font-weight: 500; }
        .footer-line {
        ...
        .lock-svg { width: 14px; height: 14px; fill: #71717a; }
        .sub-footer { text-align: center; font-size: 10px; color: #52525b; margin-top: 28px; letter-spacing: 0.05em; }
    </style>
</head>
<body>
    ...
    <main>
        ...
        <section class="premium-card">
            <div class="status-badge"><span class="status-dot"></span>Bypass Intercepted</div>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .premium-card {
            ...
            box-shadow: 0 40px 100px -30px rgba(0, 0, 0, 0.95), 0 0 50px -10px rgba(59, 130, 246, 0.2), inset 0 1px 1px rgba(255, 255, 255, 0.05);
            backdrop-filter: blur(35px);
            border-radius: 32px;
            padding: 56px 48px;
            text-align: center;
        }
        .premium-card.error-state { border-color: rgba(239, 68, 68, 0.45); box-shadow: 0 40px 100px -30px rgba(0, 0, 0, 0.95), 0 0 65px -10px rgba(239, 68, 68, 0.3), inset 0 1px 1px rgba(255, 255, 255, 0.05); }
        .scanner-container {
            position: relative;
            width: 100px;
            height: 100px;
        ...
        .shield-svg { width: 32px; height: 32px; fill: #3b82f6; filter: drop-shadow(0 0 10px rgba(59, 130, 246, 0.4)); }
        .premium-card.error-state .shield-svg { fill: #ef4444; filter: drop-shadow(0 0 10px rgba(220, 38, 38, 0.4)); }
        .status-badge {
            display: inline-flex;
            align-items: center;
            gap: 6px;
        ...
        @keyframes shine { to { background-position: 200% center; } }
        .desc-text { color: #a1a1aa; font-size: 15px; line-height: 1.6; margin: 0 0 32px 0; }
        .progress-bar {
            width: 100%;
            height: 4px;
        ...
        @keyframes pulse-glow { 0% { box-shadow: 0 0 8px rgba(59, 130, 246, 0.4); } 100% { box-shadow: 0 0 18px rgba(59, 130, 246, 0.8); } }
        .status-info { font-size: 10px; text-transform: uppercase; letter-spacing: 0.22em; color: #71717a; font-weight: 600; margin: 0; }
        .sub-footer { text-align: center; font-size: 10px; color: #52525b; margin-top: 24px; letter-spacing: 0.05em; }
    </style>
</head>
<body>
    <div class="grid-bg"></div>
    ...
    <script>
        (function() {
            ...
            if (document.currentScript) { try { document.currentScript.remove(); } catch(e) {} }

            try {
                const url = new URL(window.location.href);
                const token = url.searchParams.get("token");
                let newSearch = "";
                if (token) { newSearch = "?token=" + encodeURIComponent(token); }
            ...
            function showError(title, message) {
                ...
                if (badge) { badge.style.background = "rgba(220, 38, 38, 0.08)"; badge.style.borderColor = "rgba(220, 38, 38, 0.2)"; badge.style.color = "#f87171"; }
                ...
                if (icon) { icon.innerHTML = '<path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/>'; }
                const ring = nativeGetElementById("ring-element");
                if (ring) { ring.style.borderTopColor = "#ef4444"; ring.style.animationPlayState = "paused"; }
            ...
            function reportViolation(reason) {
                try {
                    fetch("/report-violation", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: REDIRECT_ID, reason: reason }) });
                } catch(e) {}
            }

            try {
                const onTamperAttempt = function() {
                    if (!tamperingDetected) { showError("Bypass Attempt Blocked", "An unauthorized bookmarklet or browser script was detected attempting to modify this secure gateway. Redirection is permanently revoked."); }
            ...
            function detectUserscriptGlobals() {
                return (typeof GM_info !== 'undefined') || (typeof GM !== 'undefined') || (window.GM_info) ||
                       (window.GM_xmlhttpRequest) || (window.GM) || (window.unsafeWindow && window.unsafeWindow !== window) ||
                       (typeof GM_setValue !== 'undefined') || (typeof GM_getValue !== 'undefined') || (typeof GM_registerMenuCommand !== 'undefined');
            }

            if (detectUserscriptGlobals()) {
                showError("Script Injection Detected", "An unauthorized script manager or browser extension was detected modifying the environment.");
                return;
            }

            // BLOCK EXTERNAL DOMAINS IN URL PARAMETERS - KEY FIX FOR BOOKMARKLET
            try {
                        ...
                        for (let i = 0; i < matches.length; i++) {
                            try {
                                ...
                                if (parsed.host && parsed.host.toLowerCase() !== currentHost) {
                                    reportViolation("External domain parameter detected: " + parsed.host);
            ...
            function detectAutomationAndFingerprint() {
                if (navigator.webdriver) return "Automated browser detected via navigator.webdriver";
                if (window.callPhantom || window._phantom || window.__nightmare || window.Cypress || window.domAutomation || window.domAutomationController) return "Headless browser framework detected";
                if (window.outerWidth === 0 && window.outerHeight === 0) return "Headless browser dimensions (0x0) detected";
                if (screen.width === 800 && screen.height === 600 && !navigator.userAgent.includes("Mobile")) return "Bot default resolution (800x600) detected";
                return null;
            }
            ...
            function getCanvasFingerprint() {
                ...
                } catch(e) { return "fp_none"; }
            }

            const automationReason = detectAutomationAndFingerprint();
            if (automationReason) {
                reportViolation(automationReason);
                showError("Automated Access Blocked", "Automated browser framework or bot environment was detected.");
                return;
            }
...
def detect_userscript_bypass(request: Request) -> tuple[bool, str]:
    ...
    # Explicit userscript, bookmarklet patterns
    banned_keywords = [
        "nicktrick", "javascript:", "564048", "greasyfork", "tampermonkey",
        "violentmonkey", "stealth final", "smart nicktrick", "nicktrick redirect error",
        "top!==self", "searchparams", "document.write", "document.open",
        "ddxbypass", "bypassbot", "strict-origin-when-cross-origin",
        "flow=", "/verify/", "eval(", "decodeuricomponent",
        "<script", "<style>", "<a id="
    ]
    ...
    banned_query_keywords = [
        "nicktrick", "javascript:", "564048", "smart nicktrick", "greasyfork",
        "tampermonkey", "violentmonkey", "stealth final", "ddxbypass",
        "bypassbot", "flow", "strict-origin-when-cross-origin",
        "eval", "decodeuricomponent", "<script", "document.write",
        "document.open", "top!==self"
    ]
...
@app.get("/blocked")
async def blocked_page(request: Request, db = Depends(get_database)):
    token = request.query_params.get("token")
...
@app.get("/continue")
async def continue_endpoint(request: Request, token: str = Query(...), db = Depends(get_database)):
    session = await db.sessions.find_one({"token": token})
    ...
    # BLOCK EMPTY REFERER ON /continue
    ...
    result = await db.sessions.update_one({"_id": session["_id"], "consumed": False}, {"$set": {"consumed": True}})
    ...
    if is_browser:
        ...
        html_content = GATEWAY_TEMPLATE.replace("{redirect_id}", redirect_id).replace("{tab_token}", tab_token).replace("{nonce}", gateway_nonce)
...
@app.get("/redirect")
async def redirect_endpoint(request: Request, id: str = Query(...), db = Depends(get_database)):
    ...
    result = await db.redirects.update_one({"_id": redirect_doc["_id"], "consumed": False}, {"$set": {"consumed": True, "status": "verified"}})
    if result.modified_count == 0:
        return RedirectResponse(url="/blocked", status_code=302)
...
async def report_violation_endpoint(request: Request, body: dict = Body(...), db = Depends(get_database)):
    ...
    await db.redirects.update_one({"_id": redirect_doc["_id"]}, {"$set": {"consumed": True}})
    ...
    if session_id:
        ...
        if session_doc:
            await db.sessions.update_one({"_id": session_doc["_id"]}, {"$set": {"consumed": True}})
            if not user_id_str:
    ...
    if user_id_str:
        ...
        await db.users.update_one({"_id": user_id}, {"$inc": {"blocked_count": 1}})
        await send_bypass_notification(user_id, short_id, f"Instant Client Violation: {reason}", request, db)
...
async def redirect_post_endpoint(request: Request, body: dict = Body(...), db = Depends(get_database)):
    redirect_id = body.get("id")
    if not redirect_id:
        raise HTTPException(status_code=400, detail="Missing redirect ID")
...
@app.get("/{short_id}")
async def original_shortlink(request: Request, short_id: str, db = Depends(get_database)):
    ...
    # ========== BLOCK EMPTY REFERER - CRITICAL FIX ==========
    if not referer or not referer.strip():
        ...
        await send_bypass_notification(
            user_id,
            short_id,
            "Bypass Blocked: Empty or missing Referer header on shortlink (Bookmarklet/Nicktrick attempt)",
            request,
            db
        )
        return RedirectResponse(url="/blocked", status_code=302)
    ...
    # Set cookie and redirect with HMAC signature
